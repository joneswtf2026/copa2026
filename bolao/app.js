...
function flag(time) { return BANDEIRAS[time] || '🏳'; }
...
function logLabel(type) {
  ...
  const map = {
    ...
    resultado_salvo: 'Resultado publicado',
  ...
  if (res) {
      ...
      <div class="palpite-area">
        ...
        ${pal ? `
          ...
          <div class="palpite-pontos ${acertou ? 'pontos-acertou' : 'pontos-errou'}">
            ...
            ${acertou ? '✅' : '❌'} Seu palpite: ${pal.gols1}  x  ${pal.gols2}
    ...
    palpiteArea = `
      ...
      <div class="palpite-area">
        ...
        ${pal ? `<div class="palpite-salvo">✅ Seu palpite: ${pal.gols1}  x  ${pal.gols2} ${pago ? '(pago)' : '(aguardando pagamento)'}</div>` : ''}
...
async function salvarPalpite(jogoId, gols1, gols2) {
  ...
  await logEvent('palpite_salvo', {
    ...
    descricao: `Palpite salvo para ${jogoId}: ${gols1}×${gols2} (${fase})`,
...
async function confirmarPagamento(jogoIds, total) {
  ...
  await logEvent('pagamento_solicitado', {
    ...
    quantidade: jogoIds.length,

    descricao: `Solicitação de pagamento de ${fmtBRL(total)} para ${jogoIds.length} palpites`,
...
async function renderRanking() {
  ...
  statsEl.innerHTML = `
    ...
    <div class="rank-stat-card"><div class="rank-stat-val">${totalUsuarios}</div><div class="rank-stat-lbl">Usuários cadastrados</div></div>

    <div class="rank-stat-card"><div class="rank-stat-val">${totalParticipantes}</div><div class="rank-stat-lbl">Com palpites pagos</div></div>
  ...
  container.innerHTML = `<div class="ranking-list">${ranking.map((u, i) => {
    ...
    return `
      ...
      <div class="ranking-item ${posCls}${euCls}">
...
function renderChaveamento() {
  ...
  const fases = [
    ...
    { id: 'oitavas', label: '⚙ Oitavas de Final (Round of 32)', jogos: CHAVEAMENTO_R32 },
        ...
        return `
          ...
          <div class="meu-palpite-card ${statusCls}">
            ...
            <div class="meu-palpite-detalhe">
              ...
              Palpite: <strong>${p.gols1}  x  ${p.gols2}</strong> – ${fmtData(jogo.data)} – ${faseLbl(jogo.fase || 'grupos')}

              ${res ? ` – Resultado: <strong>${res.gols1} x ${res.gols2}</strong>` : ''}

            </div>

            ${pts !== null ? `<div class="meu-palpite-pontos ${pts > 0 ? 'pontos-acertou' : 'pontos-errou'}">${pts > 0 ? ' +' + pts + ' pontos' : 'ﯔ 0 pontos'}</div>` : ''}
...
async function renderAdmin() {
  ...
  const [usersSnap, pagSnap, logsSnap] = await Promise.all([

    getDocs(collection(db, 'usuarios')),

    getDocs(collection(db, 'pagamentos')),

    getDocs(query(collection(db, 'logs'), orderBy('createdAt', 'desc'), limit(20))),
  ...
  container.innerHTML = `
    ...
    <div class="admin-section">
      ...
      <div class="admin-section-titulo">📝 Registro de Atividades</div>

      <div class="admin-card admin-log-list">
        ...
        ${logs.length > 0 ? logs.map(l => `
          ...
          <div class="admin-log-item">
            ...
            <div class="admin-log-meta">
              ...
              <span class="admin-log-type">${logLabel(l.type)}</span>

              <span>${fmtData(l.createdAt?.toDate ? l.createdAt.toDate() : l.createdAt)}</span>

            </div>

            <div class="admin-log-desc">${l.nome ? `<strong>${l.nome}</strong> – ` : ''}${l.descricao || l.email || ''}</div>

          </div>`).join('') : '<div class="empty-state-desc">Nenhuma atividade registrada ainda.</div>'}
...
async function aprovarPagamento(pedidoId, jogoIds, uid) {
  ...
  await logEvent('pagamento_aprovado', {

    uid,

    pagoPor: currentUser?.uid || '',

    pedidoId,

    total: jogoIds.length,

    quantidade: jogoIds.length,

    descricao: `Pagamento aprovado para pedido ${pedidoId} (${jogoIds.length} palpites)`,
...
async function rejeitarPagamento(pedidoId) {
  ...
  await logEvent('pagamento_rejeitado', {

    pedidoId,

    pagoPor: currentUser?.uid || '',

    descricao: `Pagamento rejeitado para pedido ${pedidoId}`,
...
async function salvarResultado(jogoId) {
  ...
  await logEvent('resultado_salvo', {
    ...
    descricao: `Resultado salvo para ${jogoId}: ${g1}×${g2}`,
